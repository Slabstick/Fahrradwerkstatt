        ...
        for (int minute = 0; minute < 60; minute++) {
          ...
          for (Mitarbeiter mitarbeiter : alleMitarbeiter) {
            if (!queue.isEmpty()) {
              ...
              if (mitarbeiter.work(time, day, hour, nextJob) == nextJob) { // wenn der Job an dem der MA gerade arbeitet
                                                                           // gleich dem ersten Eintrag der Queue ist
  ...
  public static void analyzeYear(WorkYear year) {
    ...
    for (Job job : list) {
      if (job.getWorkLeft() < 1) { // Auftrag wurde beendet
        ...
        cumulatedWaitingTime += job.getWaitingTime(); // Totale Summe aller Wartezeiten um später den Durchschnitt zu
                                                      // berechnen
